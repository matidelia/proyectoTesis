...
% Para notas
% Comentado antes de entregar (checklist del template): si el documento
% compila sin estos comandos definidos, confirma que no quedan
% comentarios de revision pendientes en ningun capitulo.
...
% \usepackage{pdfcomment} % comentado antes de entregar (checklist del template)
...
% Para fijar figuras exactamente donde se colocan en el texto (evita
% que floten lejos del titulo/parrafo que las presenta).
\usepackage{float}
...
% Espacio antes de cada entrada de capitulo en el indice: al agregar mas
% secciones el indice paso a ocupar una pagina de mas, dejando "Bibliografia"
% sola en una hoja casi en blanco. Se acota este espacio (el default de
% tocloft es mayor) para que todo el indice vuelva a entrar en las mismas
% paginas que ocupaba antes.
...
% Enlaces (citas, indice, referencias cruzadas) en negro, sin resaltado
% de color, pero siguen siendo clickeables.
\usepackage{hyperref}
\hypersetup{
    colorlinks=false,
    pdfborder={0 0 0}
...
% El entorno titlepage resetea el contador de pagina a 0 internamente,
% lo que desincroniza la numeracion impresa (indice, pies de pagina)
% respecto de la posicion fisica real dentro del PDF. Se corrige ac\'a
% para que el numero de pagina impreso coincida siempre con la pagina
% fisica que muestra cualquier lector de PDF.
\setcounter{page}{2}

% Indice inmediatamente despues de la portada.
% Agradecimientos, Resumen y Abstract quedan pendientes para la entrega
% final (no corresponden a este avance del 75%, ver README del template:
% no se hacen hasta la entrega final). Ver COMENTARIOS_REVISION.md.
\clearpage
	\tableofcontents

% La Propuesta de Tema aprobada se movio al Anexo A (documental/
% administrativa, ver chapters/appendix/proposal.tex): el cuerpo
% principal arranca directamente con el desarrollo del proyecto.
\clearpage

	\input{chapters/chapter01} % Marco Teórico
	\input{chapters/chapter02} % Estado del Arte
	\input{chapters/chapter03} % Requerimientos
	\input{chapters/chapter04} % Mockups
	\input{chapters/chapter05} % Competencias y Estrategia de Mercado
	\input{chapters/chapter06} % User Research
	\input{chapters/chapter07} % Demo - Avances de Implementación
	\input{chapters/conclusion} % Conclusion: aportes ligados a objetivos + trabajo futuro
	...
	% Bibliografia al final de todo el documento.
	\newpage
	\phantomsection
