...
En la implementación actual, el sistema identifica cada producto por el identificador de una publicación puntual de un vendedor específico (ver el diagrama Entidad-Relación, Figura~\ref{fig:diagrama-er}), no por un producto canónico que agrupe todas las publicaciones equivalentes de distintos vendedores bajo un mismo modelo -- concepto que Mercado Libre expone en su propio catálogo mediante un identificador de catálogo para los ítems de ``Compra Segura''. Esto es una decisión de diseño deliberada y no una omisión: el score de tendencia está pensado para responder la pregunta del vendedor (``¿mi publicación puntual, dentro de mi categoría, está ganando o perdiendo tracción?''), no para declarar cuál es ``el'' ganador entre publicaciones equivalentes de la competencia. El componente de ranking de la fórmula (Sección~\ref{sec:score-formal}) ya captura parte de esa dimensión competitiva, al comparar la posición del producto contra los demás de su misma categoría.

La limitación reconocida es que, si muchos vendedores publican el mismo modelo, el sistema hoy calcula un score independiente por cada publicación en lugar de un score agregado por modelo, lo que puede diluir la señal cuando la demanda de un producto se reparte entre muchas publicaciones equivalentes. La extensión planificada para resolverlo es incorporar el identificador de catálogo de la API oficial cuando esté disponible para la categoría, agregando las apariciones y el histórico de precios de todas las publicaciones que comparten ese identificador antes de calcular el score -- sin necesidad de rediseñar el modelo de datos, ya que la entidad de producto podría extenderse con ese campo opcional sin romper su relación existente con el histórico de apariciones, el histórico de precios y los scores ya calculados.
