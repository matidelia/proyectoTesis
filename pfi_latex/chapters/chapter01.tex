...
\begin{itemize}
	\item \textbf{Ya implementado (hitos 50\,\% y 75\,\%)}: captura automatizada de datos (RF01), score de tendencia basado en métricas descriptivas ponderadas -- frecuencia, permanencia, ranking y estabilidad de precio (RF02), dashboard con ranking y filtros (RF03), API REST (RF04), gráficos de evolución temporal (RF05), exportación CSV (RF06), y un primer modelo de Machine Learning supervisado (Python, Pandas + Scikit-Learn), entrenado y evaluado sobre el histórico acumulado, que clasifica productos según su probabilidad de sostener el crecimiento (Sección~\ref{sec:ml-resultados}). Este último se trata como un \emph{checkpoint} y no como versión final del modelo, porque todavía no cubre varios ciclos completos de aparición-crecimiento-declive por categoría. El sistema se encuentra desplegado en producción y capturando datos de forma autónoma.
