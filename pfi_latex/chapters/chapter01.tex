...
\begin{itemize}
	...
	\item \textbf{Implementado como checkpoint temprano}: un primer modelo de Machine Learning supervisado (Python, Pandas + Scikit-Learn), entrenado y evaluado sobre el histórico acumulado hasta la fecha, que clasifica productos según su probabilidad de sostener el crecimiento (Sección~\ref{sec:ml-resultados}). Se lo trata como un checkpoint y no como versión final porque todavía no cubre varios ciclos completos de aparición-crecimiento-declive por categoría (Sección~\ref{sec:ml-resultados}).
	\item \textbf{Planificado para el 100\,\%}: reentrenamiento y validación del modelo con más volumen histórico a medida que el sistema sigue capturando datos, análisis final de resultados y limitaciones, y redacción del informe final.
\end{itemize}
