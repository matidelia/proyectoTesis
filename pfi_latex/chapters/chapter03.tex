...
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{1.6cm} >{\raggedright\arraybackslash}p{2.6cm} >{\raggedright\arraybackslash}p{7.8cm} >{\raggedright\arraybackslash}p{2.1cm}@{}}
	...
	\midrule
	\endhead
	RF01 & Implementado & 290 productos y 5833 apariciones capturados en 72 días de operación continua (14/7 al 24/9/2026), en $\sim$206 sesiones de minería estimadas, orquestadas por GitHub Actions (\texttt{.github/workflows/mineria.yml}, desde el 18/7/2026) sin depender de una PC encendida; desde el 24/9/2026 ese mismo workflow también corre la predicción de ML y la generación de alertas de forma automática (RF07/RF09). & Sec.~\ref{sec:evidencia-cuantitativa}, Fig.~\ref{fig:diagrama-componentes} \\
	RF02 & Implementado & 11900+ scores calculados y persistidos con desglose completo. Fórmula de cinco componentes ponderados, pesos y ejemplos reales documentados. Desde el 23/09/2026 el componente de estabilidad se calcula sobre el precio promedio entre vendedores activos (no el más barato), y desde el 24/09/2026 la cantidad de vendedores activos es un componente propio ponderado (saturación); verificado con una corrida real de minería, 30 productos con datos de múltiples vendedores cargados. & Sec.~\ref{sec:score-formal} \\
	...
	RF07 & Implementado & Columna ``Prob. ML'' en el ranking con la probabilidad calculada por el checkpoint del modelo supervisado, cargada para los 243 productos con score. & Sec.~\ref{sec:ml-resultados} \\
